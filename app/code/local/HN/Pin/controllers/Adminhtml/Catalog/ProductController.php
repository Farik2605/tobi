...
class HN_Pin_Adminhtml_Catalog_ProductController extends Mage_Adminhtml_Catalog_ProductController
{
    /**
...
//            }
     ...
     * @param $qty
     */
    protected function _filterStockDataPIN(&$stockData , $qty) {
    ...
    
    {
        ...
        /** add by luuthuy205@gmail.com for synch stock*/
        if ($isEdit) {
        ...
        $pin_is_qty_sync = Mage::getStoreConfig('pin/general/general' ,$storeId);
       /** end of add by thuyluu*/
        
        $data = $this->getRequest()->getPost();
        if ($data) {
        	if ($pin_is_qty_sync == 1)  {
	        	 if (!empty($productPinInfo)) {
	        	 	 $this->_filterStockDataPIN($data['product']['stock_data'] ,$pin_qty);
	        	 } else {
